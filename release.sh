...
function create-man {
txt2man -d "$T2M_DATE" -t $T2M_NAME -r $T2M_NAME-$T2M_VERSION -s $T2M_LEVEL -v "$T2M_DESC" man/$T2M_NAME.txt > man/$T2M_NAME.$T2M_LEVEL
}
#--------------------------------------------------------

# Put here all data for your first manpage (in T2M lines)
T2M_DATE=$(LANG=C date +"%d %b %Y")
T2M_NAME=dcfldd
T2M_VERSION=$1
T2M_LEVEL=1
...
perl -i -pe "s/(?<=AC_INIT\(\[dcfldd\],\[)(.+?)(?=\].+)/$1/" configure.ac
git add configure.ac man/dcfldd.1
git commit -m "Prepare to release version $1"
git tag v$1
